...
import { CredentialModelReq } from "../models/CredentialModelReq";
import { BgImageModelRes } from "../models/BgImageModelRes";
import { LoginResponse } from "../models/LoginResponseModel";

const LoginPage: React.FC = () => {
  const [form] = Form.useForm();
  const [credentials, setCredentials] = useState<CredentialModelReq>({ username: "", password: "" });
  const [bgImage, setBgImage] = useState<BgImageModelRes>({ bannerId: null, bannerName: "", bannerUrl: "" });
  ...
  const handleLogin = async (values: CredentialModelReq) => {
    try {
      console.log("Username:", values.username);
      console.log("Password:", values.password);

      const { data } = await api.post<LoginResponse>(API_ENDPOINTS.LOGIN, values);
  ...
  return (
    <Layout
      ...
      style={{ backgroundImage: `url("${bgImage?.bannerUrl || ""}")` }}
    >
      <Form
      ...
      >
        ...
        <Form.Item>
          <Form.Item name="remember" valuePropName="checked" noStyle>
            <Checkbox>
              <span className="cl-w">Ghi nhớ đăng nhập</span>
            </Checkbox>
          </Form.Item>
